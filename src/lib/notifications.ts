...
export class NotificationService {
  ...
  public showPomodoroNotification(type: 'focus-complete' | 'break-complete', sessionCount?: number) {
    // Add in-app notification for pomodoro completion
    if (globalAddNotification) {
      const title = type === 'focus-complete' ? 'Focus Session Complete! 🎉' : 'Break Time Over! ⚡';
      const body = type === 'focus-complete' 
        ? `Great work! Time for a well-deserved break. Sessions completed: ${sessionCount || 0}`
        : 'Ready to focus again? Let\'s get back to work!';
      
      this.addInAppNotification(
        '', // Will be set by the context
        title,
        body,
        'pomodoro-complete',
        { sessionCount, type }
  ...
  public showTaskCompleteNotification(taskTitle: string) {
    // Add in-app notification for task completion
    if (globalAddNotification) {
      this.addInAppNotification(
        '', // Will be set by the context
        'Task Completed! 🎯',
