...
export const ContributionGraph: React.FC = () => {
  ...
  const getActivityColor = (level: number) => {
    switch (level) {
      ...
      case 1: return 'bg-success/30 dark:bg-success/40';
      case 2: return 'bg-success/50 dark:bg-success/60';
      case 3: return 'bg-success/70 dark:bg-success/80';
      case 4: return 'bg-success dark:bg-success';
                  ...
                  <div key={weekIndex} className="flex flex-col gap-1">
                    {week.map((dayData, dayIndex) => (
                      <div
                        key={dayIndex}
                        className={cn(
                          "rounded-sm cursor-pointer transition-all active:scale-95",
                          "w-2.5 h-2.5 sm:w-3 sm:h-3", // Smaller on mobile
                          "hover:ring-2 hover:ring-primary/50 touch-manipulation",
