...
import { 
  ...
  Check,
  X,
  Activity // Assuming this was missing from the original import list
} from "lucide-react";
import { useTheme } from "next-themes";
import { toast } from "@/components/ui/use-toast"; // Assuming toast is used
...
export default function ProfilePage() {
  ...
  }, [user, userProfile?.teamId]); // Added userProfile.teamId dependency
  ...
  const handleAcceptRequest = async (requestId: string, senderId: string) => {
    try {
      ...
      await loadTeamMembers(); // Reload members after accepting
  ...
  const profileForm = useForm<z.infer<typeof profileSchema>>({
    ...
    values: { // Use values to reflect updates
      displayName: userProfile?.displayName || "",
      email: user?.email || "",
    }
  });
  ...
  const handlePreferencesUpdate = async (values: z.infer<typeof preferencesSchema>) => {
    try {
      ...
      // Build the preferences object with required fields
      const preferences = {
